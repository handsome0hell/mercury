...
use core_rpc_types::axon::{
    ...
    IssueAssetPayload, StakeTokenPayload, SubmitCheckpointPayload, UnlockWithdrawalPayload,
    UpdateCheckpointPayload, UpdateStakePayload,
};
...
impl<C: CkbRpc> MercuryRpcServer for MercuryRpcImpl<C> {
    ...
    async fn build_stake_token_transaction(
        &self,
        payload: StakeTokenPayload,
    ) -> RpcResult<TransactionCompletionResponse> {
        rpc_impl!(self, inner_stake_token, payload)
    }
    ...
    async fn build_unlock_withdrawal_transaction(
        ...
        rpc_impl!(self, inner_unlock_withdrawal, payload)
    }

    async fn build_burn_withdrawal_transaction(
        &self,
        payload: BurnWithdrawalPayload,
    ) -> RpcResult<TransactionCompletionResponse> {
        rpc_impl!(self, inner_burn_withdrawal, payload)
    }
